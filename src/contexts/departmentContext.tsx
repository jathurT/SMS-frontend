...
type DepartmentAction =
  ...
  | { type: "FETCH_ANALYTICS_BY_ID"; payload: DepartmentAnalyticsDetail }
...
const departmentReducer = (
  ...
  switch (action.type) {
    ...
    case "FETCH_ANALYTICS_BY_ID":
      return {
        ...state,
        analytics: state.analytics.map((item) =>
...
interface DepartmentContextType {
  ...
  fetchDepartmentAnalyticsById: (
    id: number
  ) => Promise<DepartmentAnalyticsDetail>;
}
...
export const DepartmentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const createDepartment = useCallback(
    async (department: CreateDepartment): Promise<Department> => {
      try {
        dispatch({ type: "SET_ERROR", payload: null });
        dispatch({ type: "SET_LOADING", payload: true });
  ...
  const updateDepartment = useCallback(
    async (id: number, department: CreateDepartment): Promise<Department> => {
      try {
        dispatch({ type: "SET_ERROR", payload: null });
        dispatch({ type: "SET_LOADING", payload: true });
