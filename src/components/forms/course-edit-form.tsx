...
function CourseEditForm({ 
  ...
  const { state, updateCourse, fetchCourseById } = useCourseContext();
  ...
  useEffect(() => {
    const loadCourse = async () => {
      try {
        ...
        // First check if course exists in current state
        ...
        if (existingCourse) {
          ...
          const department = departmentState.departments.find(
            dept => dept.departmentName === existingCourse.departmentName
          );
          
          setFormData({
            courseName: existingCourse.courseName,
            courseCode: existingCourse.courseCode,
            enrollmentKey: existingCourse.enrollmentKey || "",
          ...
          // Find department ID from department name
          const department = departmentState.departments.find(
            dept => dept.departmentName === course.departmentName
          );
    ...
    // Only load course if departments are already loaded
    if (departmentState.departments.length > 0 || !departmentState.loading) {
      loadCourse();
    }
  }, [courseId, state.courses, fetchCourseById, setIsOpen, error, departmentState.departments, departmentState.loading]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    info({
      title: "Processing...",
      description: "Updating course information",
      duration: 2000
    });
    ...
    try {
      ...
      error({
        title: "Update Failed",
        description: errorMessage || state.error || "Failed to update course",
        duration: 6000
      });
    } finally {
      setIsLoading(false);
  ...
  if (isLoadingData || departmentState.loading) {
    return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="semester">
            Semester <span className="text-red-2000">*</span>
          </Label>
          <Input
            id="semester"
            type="text"
