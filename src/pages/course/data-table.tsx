...
export function DataTable<TData extends CourseData, TValue>({
  columns,
  data,
  enableRowClick = true,
  detailsRoutePath = "/course",
  ...
  const handleRowClick = (courseId: number, event: React.MouseEvent) => {
    ...
    if (!isInteractiveElement) {
      navigate(`${detailsRoutePath}/${courseId}`);
    }
  };
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={(e) => handleRowClick(row.original.courseId, e)}
