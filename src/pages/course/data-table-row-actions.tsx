...
import CourseDeleteForm from "@/components/forms/course-delete-form";
...
export function DataTableRowActions<TData extends WithId<string>>({
  ...
  return (
    <>
      ...
      >
        <CourseEditForm courseId={courseId} setIsOpen={setIsEditOpen} />
      </ResponsiveDialog>
      <CourseDeleteForm
        courseId={courseId}
        isOpen={isDeleteOpen}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-[160px] z-50">
            ...
            <DropdownMenuItem className="group flex w-full items-center justify-between text-left p-0 text-sm font-base">
              <button
                onClick={() =>
                  navigator.clipboard.writeText(courseId.toString())
                }
                className="w-full justify-start flex rounded-md p-2 transition-all duration-75"
              >
...
export const useRowNavigation = (courseId: number) => {
  const navigate = useNavigate();
