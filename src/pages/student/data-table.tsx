...
export function DataTable<TData extends StudentData, TValue>({
  columns,
  data,
  enableRowClick = true,
  detailsRoutePath = "/student",
  ...
  const handleRowClick = (studentId: number, event: React.MouseEvent) => {
    ...
    if (!isInteractiveElement) {
      navigate(`${detailsRoutePath}/${studentId}`);
    }
  };
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={(e) => handleRowClick(row.original.studentId, e)}
